...
void ThreadJobEnchRho(mpz_t output, mpz_t to_factor, volatile bool& factored, int seed){
...
void PollardsRhoEnch(mpz_t output, mpz_t to_factor, int thread_count){
    if(thread_count == 0){
        thread_count = 8;
    }
    ...
    if(thread_count > 1){
        ...
        for(int i = 0; i < thread_count; i++){
            ...
            workers.emplace_back(std::thread(ThreadJobEnchRho, std::ref(output), std::ref(to_factor), std::ref(factored), random_seed));
        }
        ...
        int random_seed = rand();
        ThreadJobEnchRho(output, to_factor, factored, random_seed);
    }
}